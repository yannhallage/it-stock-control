...
import { useDepartments, type Department } from '../api/hooks/useDepartments'
...
export function DepartmentsPage() {
  ...
  const { fetchDepartments, createDepartment, updateDepartment, deleteDepartment, loading, error: apiError } =
    useDepartments()
  ...
  useEffect(() => {
    ...
    fetchDepartments()
      ...
      .catch((e) => {
        ...
        toast.error(msg || 'Erreur lors du chargement des départements.')
      })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedName) {
      toast.warning('Le nom du département est obligatoire.')
      return
    }

    try {
      if (editingId !== null) {
        const updated = await updateDepartment(editingId, { name: trimmedName })
      ...
      const msg = errorMessageFromUnknown(e, "Erreur lors de l'enregistrement du département.")
      setError(msg)
      toast.error(msg)
    }
  }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Supprimer ce département ?')) {
      ...
      try {
        ...
        const msg = errorMessageFromUnknown(e, 'Erreur lors de la suppression du département.')
        setError(msg)
        toast.error(msg)
      }
    }
            ...
            <td colSpan={2} className="px-4 py-8 text-center text-gray-500">
              {items.length === 0 ? (
                loading ? (
                  ...
                  'Aucun département. Cliquez sur « Ajouter un département » pour commencer.'
                )
              ) : (
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold text-gray-900">
              {editingId !== null ? 'Modifier un département' : 'Ajouter un département'}
            </h2>
